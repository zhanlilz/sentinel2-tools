...
int read_one_row_gdal(GDALDatasetH hDataset, int iBand, int iRow, void *buf)
{
        ...
        if(CE_None != GDALRasterIO(hBand, GF_Read, 0, iRow, ncol, 1, buf, ncol, 1, dtype, 0, 0)){
...
int read_one_band_gdal(GDALDatasetH hDataset, int iBand, void *buf)
{
        if(buf == NULL){
                return -1;
        }       
        ...
        if(CE_None != GDALRasterIO(hBand, GF_Read, 0, 0, ncol, nrow, buf, ncol, nrow, dtype, 0, 0)){
...
int find_next_element(char *strXml, char *element)
{
...
int get_element_value(char *strXml, char *element, char *property, char *value)
{
        int i, j;
        int p = 0;
        int len = strlen(strXml);
...
int get_srf_band_index(int instrument, char *bname)
{
        char str_srf_bands_tm[6][20] = {"sr_band1","sr_band2","sr_band3","sr_band4","sr_band5","sr_band7"};
        char str_srf_bands_oli[6][20] = {"sr_band2","sr_band3","sr_band4","sr_band5","sr_band6","sr_band7"};
...
int parse_usgs_xml(char *fname, usgs_t *scene)
{
        ...
        if(0 != get_element_value(&(strXml[a]), "solar_angles", "azimuth", strTmp)){
                free(strXml);
                return -3;
        }
        ...
        if(0 != get_element_value(&(strXml[a]), "projection_information", "projection", scene->projection)){
                free(strXml);
                return -3;
        }
...
void get_year_doy(char *str, int *year, int *doy)
{
        char cmd[1024];
...
int parse_sentinel_xml(char *fname, usgs_t *scene)
{
        ...
        if(0 != get_element_value(&(strXml[a]), "SENSING_TIME metadataLevel=\"Standard\"", NULL, strTmp)){
                free(strXml);
                return -3;
        }
        ...
        if(0 != get_element_value(&(strXml[a]), "HORIZONTAL_CS_NAME", NULL, strTmp)){
                free(strXml);
                return -3;
        }
        ...
        if(0 != get_element_value(&(strXml[a]), "ZENITH_ANGLE unit=\"deg\"", NULL, strTmp)){
                free(strXml);
                return -3;
        }
        scene->szn = atof(strTmp);

        if(0 != get_element_value(&(strXml[a]), "AZIMUTH_ANGLE unit=\"deg\"", NULL, strTmp)){
                free(strXml);
                return -3;
        }
        scene->san = atof(strTmp);
        ...
        for(i=0; i<6; i++){
                ...
                sprintf(scene->band[i].file_name, "%s/IMG_DATA/R20m/S2A_USER_MSI_L2A_TL_%s__%s_%s_%s_20m.jp2", dir, str_id3, str_id, str_id2, bands[i]);
        ...
        sprintf(scene->band[i].file_name, "%s/IMG_DATA/S2A_USER_SCL_L2A_TL_%s__%s_%s_20m.jp2", dir, str_id3, str_id, str_id2);
        ...
        sprintf(scene->band[i].file_name, "%s/QI_DATA/S2A_USER_CLD_L2A_TL_%s__%s_%s_20m.jp2", dir, str_id3, str_id, str_id2);
        ...
        sprintf(scene->band[i].file_name, "%s/QI_DATA/S2A_USER_SNW_L2A_TL_%s__%s_%s_20m.jp2", dir, str_id3, str_id, str_id2);
...
int convert_to_binary(usgs_t *scene)
{
        ...
        for(i=0; i<scene->nband; i++){
                sprintf(cmd, "if [ ! -r %s.bin ]; then \ngdal_translate -of ENVI %s %s.bin \nfi", 
                                                                                scene->band[i].file_name, scene->band[i].file_name, scene->band[i].file_name);
                if(0 != system(cmd)){
                        return -1;
                }       
...
int read_a_band_a_row(band_t *band, int row, void *buf)
{
        if(band->file_format == TIF){
                ...
                if(band->ncol != fread(buf, band->nbyte_per_pix, band->ncol, band->fp)){
                        printf("read band %s row %d failed.\n", band->file_name, row);
                        return -2;
                }       
...
int read_a_band(band_t *band, void *buf)
{
        if(band->file_format == TIF){
                ...
                if(band->ncol*band->nrow != fread(buf, band->nbyte_per_pix, band->ncol*band->nrow, band->fp)){
                        printf("read band %s row %d failed.\n", band->file_name, row);
                        return -2;
                }       
                //Note: binary file from netwrok is big-endian
