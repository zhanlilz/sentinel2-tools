...
int parse_sentinel_xml(char *fname, usgs_t *scene)
{
	...
	for(i=0; i<6; i++){
		...
		scene->band[i].scale_factor = 0.0005;
		sprintf(scene->band[i].file_name, "%s/IMG_DATA/R20m/S2A_USER_MSI_L2A_TL_MTI__%s_%s_%s_20m.jp2", dir, str_id, str_id2, bands[i]);
	...
	sprintf(scene->band[i].file_name, "%s/QI_DATA/S2A_USER_CLD_L2A_TL_MTI__%s_%s_20m.jp2", dir, str_id, str_id2);
	...
	sprintf(scene->band[i].file_name, "%s/QI_DATA/S2A_USER_SNW_L2A_TL_MTI__%s_%s_20m.jp2", dir, str_id, str_id2);
