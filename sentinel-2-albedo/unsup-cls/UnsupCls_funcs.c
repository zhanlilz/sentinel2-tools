...
int
SubString (char *str_ptr, unsigned int str_start, unsigned int str_end,
					 char *p)
/*
!C***********************************************************************
...
OpenLndsrFile (PARM_T * parm)
{
	...
	double ulx, uly, res;
	if (0 !=
			get_scene_proj (&(parm->scene), &sys, &zone, &datum, pparm, &ulx, &uly,
											&res))
		{
  ...
  */
{
	...
	if (sds_index == FAILURE)
		{
			fprintf (stderr,
							 "\nOpenLndsrFile():fail to get the identifier for SDS:%s.",
							 sdsName[0]);
			return UNSCLS_FAIL;
		}
	else
		{
			...
			if (sds_id == FAILURE)
				{
					fprintf (stderr, "\nOpenLndsrFile():fail to select the SDS:%s.",
									 sdsName[0]);
					return UNSCLS_FAIL;
				}
			else
				{
					status =
						SDgetinfo (sds_id, name, &rank, dimisizes2, &SDdata_type,
											 &n_attrs);
					if (status != FAILURE)
						{
	...
	for (i = 0; i < parm->nbands; i++)
		{
			...
			if (sds_index == FAILURE)
				{
					fprintf (stderr,
									 "\nOpenLndsrFile():fail to acquire the identifier for SDS:%s.",
									 sdsName[i]);
					return UNSCLS_FAIL;
				}
			else
	...
	for (i = 0; i < N_SRF_BAND; i++)
		{
			...
			for (row = 0; row < parm->nlines; row++)
				{
					if (0 !=
							read_a_band_a_row (&(parm->scene.band[index]), row,
																 (void *) OneRowBuffer))
						{
							printf ("Read failed.\n");
							free (OneRowBuffer);
					...
					if (0 !=
							read_a_band_a_row (&(parm->scene.band[index_qa]), row,
																 (void *) OneRowBuffer_qa))
						{
		...
		for (irow = 0; irow < parm->nlines; irow++)
			{
				...
				if (SDreaddata (parm->sr_id[iband], start, NULL, length, OneRowBuffer)
						== FAILURE)
					{
						fprintf (stderr, "\nOpenLndsrFile():fail to read the %dth SDS.",
										 iband);
						flg = UNSCLS_FAIL;
						break;
					}
...
int
ShortestDisCls (int row, int col, PARM_T * parm)
/*
!C***********************************************
			...
			if (icls != -1)
				{
					for (iband = 0; iband < parm->nbands; iband++)
						{
							tmp_dis =
								parm->image[row][col][iband] * parm->scalefactor -
								parm->mean_x[icls][iband];
					...
					for (iband = 0; iband < parm->nbands; iband++)
						{
							...
							parm->mean_x[icls_a][iband] =
								parm->sum_x[icls_a][iband] / parm->c_npixels[icls_a];
						}
					available_icls--;
				}
			else
...
/**************************************************************************/
int
Find2ClsWithMiniDis (int *loc_a, int *loc_b, PARM_T * parm)
{
	int band;
				...
				if ((cls1 != -1) && (cls2 != -1))
					{
						...
						for (band = 0; band < parm->nbands; band++)
							{
								tmp_diff =
									parm->mean_x[cls1][band] - parm->mean_x[cls2][band];
...
*/
{
	...
	for (iband = 0; iband < parm->nbands; iband++)
		if ((SDendaccess (parm->sr_id[iband])) == FAILURE)
			{
				fprintf (stderr, "\nCloseLndsr():SDendaccess for lndar band%d error!",
								 iband);
				return UNSCLS_FAIL;
			}
	if ((SDend (parm->fp_id)) == FAILURE)
		{
			fprintf (stderr, "\nCloseLndsr():SDend for file %s error!",
							 parm->filename);
			return UNSCLS_FAIL;
		}
...
void
alloc_1dim_contig (void **ptr, int d1, int elsize)
{
	void *p = NULL;
...
void
alloc_2dim_contig (void ***ptr, int d1, int d2, int elsize)
{
	void *p = NULL;
	void **pp = NULL;
	int i = 0;
...
void
alloc_3dim_contig (void ****ptr, int d1, int d2, int d3, int elsize)
{
	void *p = NULL;
	void **pp = NULL;
	void ***ppp = NULL;
...
void
alloc_4dim_contig (void *****ptr, int d1, int d2, int d3, int d4, int elsize)
{
	void *p = NULL;
	void **pp = NULL;
	void ***ppp = NULL;
...
int
AllocateEarlyVars (PARM_T * parm)
{
	/* alloc the memory  to variables */
	alloc_3dim_contig ((void ****) (&parm->image), parm->nlines, parm->nsamps,
										 parm->nbands, sizeof (int16));

	alloc_1dim_contig ((void **) (&parm->c_npixels), parm->nclusters,
										 sizeof (int));
	alloc_1dim_contig ((void **) (&parm->c_order), parm->nclusters,
										 sizeof (int));
	alloc_2dim_contig ((void ***) (&parm->sum_x), parm->nclusters, parm->nbands,
										 sizeof (long double));
	alloc_2dim_contig ((void ***) (&parm->mean_x), parm->nclusters,
										 parm->nbands, sizeof (long double));
	alloc_3dim_contig ((void ****) (&parm->sum_xy), parm->nclusters,
										 parm->nbands, parm->nbands, sizeof (long double));

	return UNSCLS_SUCCESS;
}

void
free_2dim_contig (void **a)
{
	free (a[0]);
	free (a);
	return;
