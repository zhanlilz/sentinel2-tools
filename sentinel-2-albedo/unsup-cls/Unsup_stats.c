...
void accum (int cndx, int16 * pixel, PARM_T * parm)
{
	...
	for (band = 0; band < parm->nbands; band++)
	{
		...
		{
			parm->sum_xy[cndx][band][band2] += p_band * pixel[band2] * parm->scalefactor;
		}

		parm->mean_x[cndx][band] = parm->sum_x[cndx][band] / parm->c_npixels[cndx];
	}
...
int find_cluster (int l, int s, PARM_T * parm)
{
	...
	for (i = 0; i < parm->iclusters; i++)
	{
		...
		for (band = 0; band < parm->nbands; band++)
		{
			diff = parm->mean_x[iclus][band] - parm->image[l][s][band] * parm->scalefactor;
...
void mcov (PARM_T * parm)
{
	...
	for (iclass = 0; iclass < parm->nclasses && iclass < parm->iclusters; ++iclass)
	{
		...
		{
			printf ("%.*lg%s", DBL_DIG,
				parm->sum_x[clus][band] / parm->c_npixels[clus], band == parm->nbands - 1 ? "\n\n" : " ");
		}
/**
				...
				printf ("%.*lg%s", DBL_DIG,
					(parm->sum_xy[clus][band][band2] -
						((parm->sum_x[clus][band] *
							...
							parm->c_npixels[clus])) / parm->c_npixels[clus], band2 == band ? "\n" : " ");
			}
		}
