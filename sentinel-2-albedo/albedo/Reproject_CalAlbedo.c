...
int
ReprojectLnd2MODIS (DIS_LANDSAT * sensor, GRID_MODIS * modis)
{
	int i, j, k, band;
	/*int agg_exist; */
	int min_count;
	...
	alloc_3dim_contig ((void ****) (&mcls), modis->nrows, modis->ncols,
										 sensor->init_ncls + 1, sizeof (int16));
	alloc_2dim_contig ((void ***) (&count), modis->nrows, modis->ncols,
										 sizeof (int16));
	alloc_2dim_contig ((void ***) (&AvaPix_cnt), sensor->init_ncls + 1,
										 MODIS_NBANDS, sizeof (long int));
	...
	min_count =
		(int) ((modis->res * modis->res) / (sensor->res * sensor->res) * 0.8);

	if (Sensor2MODIS
			(sensor, sensor->nrows / 2, sensor->ncols / 2, modis, &mi,
			 &mj) == SUCCESS)
		{
			...
			for (j = 0; j < sensor->ncols; j++)
				{
					...
					if ((icls > sensor->actu_ncls + 1)
							|| ((icls < 0) && (icls != sensor->Cls_fillValue)))
						{
							fprintf (stderr,
											 "\nClass#%d is out of scope at (row:%d,col:%d)!", icls,
											 i, j);
							return FAILURE;
						}
		...
		for (j = 0; j < modis->ncols; j++)
			{
				...
				if ((max_percent != -1) && (count[i][j] >= min_count)
						&& (maj_cls !=
								sensor->Cls_fillValue) /*&&(maj_cls<sensor->actu_ncls+1) */ )
					{
						/*make sure enough pixels >80% fallend in the MODIS bin */
						modis->major_cls[i][j] = (int8) maj_cls;
						modis->percent_cls[i][j] =
							(uint8) (max_percent * 100.0 / count[i][j] + 0.5);
		...
		for (j = 0; j < modis->ncols; j++)
			{
				...
				fwrite (&(modis->percent_cls[i][j]), sizeof (uint8), 1,
								modis->agg_fp);
			}
		...
		for (j = 0; j < modis->ncols; j++)
			{
				...
				if ((icls != sensor->Cls_fillValue)	/*&&(icls>0)&&(icls<=sensor->actu_ncls) */
						&& (sensor->pure_thrsh[icls] != -1)
						&& (modis->percent_cls[i][j] >= sensor->pure_thrsh[icls]))
					{
								...
								for (band = 0; band < MODIS_NBANDS; band++)
									{
										...
										if (tmp <= 1 && modis->Pix_brdf[band][0] <= 1000
												&& modis->Pix_brdf[band][1] <= 1000
												&& modis->Pix_brdf[band][2] <= 1000
												&& modis->Pix_brdf[band][0] >= 0
												&& modis->Pix_brdf[band][1] >= 0
												&& modis->Pix_brdf[band][2] >= 0)
											/*if ((tmp<1)&& (!((modis->Pix_brdf[band][1]==0)&&(modis->Pix_brdf[band][2]==0)))) */
											{
												AvaPix_cnt[icls][band]++;
												sensor->BRDF_paras[icls][band][0] +=
													modis->Pix_brdf[band][0];
												if (modis->Pix_brdf[band][0] > 1000)
													{
														printf
															("XXXXXXXXXXX ISO=%d, i=%d, j=%d, band=%d, bits=%d,%d,%d,%d, tmp=%d,qa=%d, bits=",
															 modis->Pix_brdf[band][0], i, j, band,
															 bitp[k - 4], bitp[k - 3], bitp[k - 2],
															 bitp[k - 1], tmp, modis->Pix_qa);
														int aaa;
												...
												sensor->BRDF_paras[icls][band][1] +=
													modis->Pix_brdf[band][1];
												sensor->BRDF_paras[icls][band][2] +=
													modis->Pix_brdf[band][2];
											}
									}							/*end for (band=0... */
							}									/* if (modis->qa_id!=FAILURE... */
...
/******************************************************************************/
int
PrepareCalculation (DIS_LANDSAT * sensor)
{
	int i, j, k;
	int8 Pix_QA;
	...
	for (i = 0; i < sensor->nrows; i++)
		{
			...
			for (j = 0; j < sensor->ncols; j++)
				{
					...
					if (i == DEBUG_irow && j == DEBUG_icol)
						{
							printf ("qa band file name = %s\n",
											sensor->scene.band[sensor->scene.qa_band_index].
											file_name);
							printf ("i=%d, j=%d, unsup class value = %d\n", i, j,
											sensor->OneRowlndSRQAData[j]);
							int zl;
							for (zl = 0; zl < N_SRF_BAND; zl++)
								{
									printf ("sr band file name = %s\n",
													sensor->scene.band[sensor->scene.
																						 srf_band_index[zl]].file_name);
									printf ("sr, band %d, i=%d, j=%d, val=%d\n", zl, i, j,
													sensor->OneRowlndSR[zl][j]);
								}
					...
					if ((CalculateAlbedo
							 (sensor, tmplndPixAlbedo, i, j, &Pix_QA, SnowFlag)) == SUCCESS)
						{
							for (k = 0; k < 18; k++)
								{
									lndPixAlbedo[k] =
										(int16) ((10000.0 * tmplndPixAlbedo[k] + 0.5) / 1.0);
								}
							for (k = 0; k < 6; k++)
								{
									sensor->OneRowlndSpectralAlbedo[k][2 * j] =
										lndPixAlbedo[k * 2];
									sensor->OneRowlndSpectralAlbedo[k][2 * j + 1] =
										lndPixAlbedo[k * 2 + 1];
								}
						}
					else
			...
			fw_num =
				fwrite (sensor->OneRowlndSWalbedo, sizeof (int16), sensor->ncols * 2,
								sensor->fplndalbedo);
			...
			fw_num =
				fwrite (sensor->OneRowlndQA, sizeof (int8), sensor->ncols,
								sensor->fplndQA);
				...
				for (k = 0; k < 6; k++)
					{
						...
						fw_num =
							fwrite (&tmp, sizeof (int16), 1, sensor->fpSpectralAlbedo);
						if (fw_num != 1)
							{
								fprintf (stderr,
												 "\n Failed to write out lndSpectralAlbedo for row:%d,col:%d.",
												 i, j);
								return FAILURE;
							}
