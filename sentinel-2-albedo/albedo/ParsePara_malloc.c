...
int
parseParameters (char *ifile, DIS_LANDSAT * sensor, GRID_MODIS * modis)
{
	FILE *in;
	int i;
	...
	char *label = NULL;
	char *tokenptr = NULL;
	char *seperator = "= ,", *p1 = ".MajCls", *p2 = ".Majhis", *p3 =
		"_albedo.bin", *p4 = "_QA.bin";
			...
			while (tokenptr != NULL)
				{
					...
					if (strcasecmp (label, "DM_CLSMAP") == 0)
					...
					else if (strcasecmp (label, "OUTPUT_ALBEDO") == 0)
						{
							...
							strncpy (oldlndalbedoName, sensor->lndalbedoName,
											 strlen (sensor->lndalbedoName));
#ifdef SNOW
							sprintf (sensor->lndalbedoName, "%s_snow",
											 sensor->lndalbedoName);
#endif
						}
...
int
substr (char *str, char *s, int begin, int end)
{
	char *p;
	int i;
...
void
dec2bin (unsigned int num, int *bitpattern, int limit)
{
