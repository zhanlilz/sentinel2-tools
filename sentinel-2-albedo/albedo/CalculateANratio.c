...
/*******************************************************/
int
CalculateANratio (int16 * brdf, double SZA, double SAA, double VZA,
									double VAA, double *ANR_wsa, double *ANR_bsa)
{
	double wsa, wsa_weight[3] = { 1.0, 0.189184, -1.377622 };
	double bsa;
	double bsk_RossThick[91] =
		...
		1.184742, 1.279047, 1.394945,
		1.568252
	};
	double bsk_LiSparseR[91] =
		...
		-1.510596, -1.516135, -1.526204, -1.555139,
		-37286.245624
	};
	double ref_dir;
	...
	bs_LSRKer =
		bsk_LiSparseR[thisang - 1] + (bsk_LiSparseR[thisang] -
																	bsk_LiSparseR[thisang - 1]) * angdis;
	bs_RTKer =
		bsk_RossThick[thisang - 1] + (bsk_RossThick[thisang] -
																	bsk_RossThick[thisang - 1]) * angdis;
	...
	VZA = VZA * D2R;
	wsa =
		brdf_scale * (wsa_weight[0] * brdf[0] + wsa_weight[1] * brdf[1] +
									wsa_weight[2] * brdf[2]);
	bsa =
		brdf_scale * (1.0 * brdf[0] + bs_RTKer * brdf[1] + bs_LSRKer * brdf[2]);

	k_iso = 1.0;
	k_geo = LiSparseR_kernel (VZA, SZA, phi);
	k_vol = RossThick_kernel (VZA, SZA, phi);
	ref_dir =
		brdf_scale * (k_iso * brdf[0] + k_vol * brdf[1] + k_geo * brdf[2]);
	...
	if (_ANR_bsa > 4 || _ANR_wsa > 4)
		{
			printf
				("ANR_BSA=%f,ANR_WSA=%f,SZA=%f,VZA=%f,WSA=%f,BSA=%f,NBAR=%f,BRDF=%d,%d,%d,KER=%f,%f,%f\n",
				 _ANR_bsa, _ANR_wsa, SZA * R2D, VZA * R2D, wsa, bsa, ref_dir, brdf[0],
				 brdf[1], brdf[2], k_iso, k_geo, k_vol);
		}
