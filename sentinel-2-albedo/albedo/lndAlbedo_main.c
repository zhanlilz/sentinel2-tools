...
int main(int argc, char *argv[])
{
  ...
  alloc_3dim_contig((void ****)(&(sensor->BRDF_paras)), sensor->init_ncls+1, MODIS_NBANDS, 3, sizeof(double));

  /* Allocate memory for one Row output land SW-albedo, QA, and spectral albedo*/
  alloc_1dim_contig((void **)(&(sensor->OneRowlndSWalbedo)),  sensor->ncols*2, sizeof(int16));
  alloc_1dim_contig((void **)(&(sensor->OneRowlndQA)),  sensor->ncols, sizeof(int8));
  alloc_2dim_contig((void ***)(&(sensor->OneRowlndSpectralAlbedo)),  6, sensor->ncols*2, sizeof(int16));
...
int subset_modis(DIS_LANDSAT *sensor, GRID_MODIS *modis)
{
	if(0 != SetupSpace(sensor->insys, sensor->inzone, sensor->inparm, sensor->indatum, 
													sensor->ulx, sensor->uly, (double)sensor->res)){
	...
	printf("Landsat scene center: lat=%f, lon=%f, Date: %d-%03d\n", lat, lon, sensor->scene.year, sensor->scene.doy);
	char cmd[1024];
	char *dir = "/neponset/nbdata05/albedo/qsun/V6_OUT";
	char *exe="/home/qsun/code/mosaic_cut_modis/sub";
	char out[1024];
	char out_qa[1024];
	sprintf(out, "./runtime/mcd43a1_sub_A%d%03d_lat%f_lon%f.hdf", sensor->scene.year, sensor->scene.doy, lat, lon);
	sprintf(out_qa, "./runtime/mcd43a2_sub_A%d%03d_lat%f_lon%f.hdf", sensor->scene.year, sensor->scene.doy, lat, lon);
	sprintf(cmd, "%s %s MCD43A1 %f %f %d %d %s", exe, dir, lat, lon, sensor->scene.year, sensor->scene.doy, out);
	assert(0 == system(cmd));
	sprintf(cmd, "%s %s MCD43A2 %f %f %d %d %s", exe, dir, lat, lon, sensor->scene.year, sensor->scene.doy, out_qa);
	assert(0 == system(cmd));
