...
int
loadSensorClsRow (DIS_LANDSAT * sensor, int irow)
{
	int temp;
	long offset;
	//int8 mycls;
...
int
load_lndSRrow (DIS_LANDSAT * sensor, int row)
{
	int i;
	int index;
	...
	for (i = 0; i < N_SRF_BAND; i++)
		{
			...
			if (0 !=
					read_a_band_a_row (&(sensor->scene.band[index]), row,
														 (void *) sensor->OneRowlndSR[i]))
				{
					printf ("Read failed.\n");
					return FAILURE;
				}
	...
	if (0 !=
			read_a_band_a_row (&(sensor->scene.band[index]), row,
												 (void *) sensor->OneRowlndSRCldData))
		{
			printf ("Read failed.\n");
			return FAILURE;
		}
	...
	if (0 !=
			read_a_band_a_row (&(sensor->scene.band[index]), row,
												 (void *) sensor->OneRowlndSRSnwData))
		{
			printf ("Read failed.\n");
			return FAILURE;
		}
...
int
load_lndSRrow_old (DIS_LANDSAT * sensor, int irow)
{
	...
	for (iband = 0; iband < SENSOR_BANDS; iband++)
		{
			...
			if (SDreaddata (sds_id, start, NULL, length, sensor->OneRowlndSR[iband])
					== FAILURE)
				{
					fprintf (stderr, "ERROR:  detected FAIL\n"
									 "\t from HDF procedure SDreaddata\n"
	...
	if (SDreaddata
			(sensor->lndatmo_id, start, NULL, length,
			 sensor->OneRowlndSRAtmoData) == FAILURE)
		{
			fprintf (stderr, "\nload_lndSRrow():fail to read LndSRAtmo at row:%d.",
							 irow);
			exit (1);
		}
	if (SDreaddata
			(sensor->lndQA_id, start, NULL, length,
			 sensor->OneRowlndSRQAData) == FAILURE)
		{
			fprintf (stderr, "\nload_lndSRrow():fail to read LndSRQA at row:%d.",
							 irow);
			exit (1);
		}
...
int
loadSensorSRPixel (DIS_LANDSAT * sensor, int irow, int icol)
{
	...
	for (iband = 0; iband < SENSOR_BANDS; iband++)
		{
			...
			length[0] = 1;
			length[1] = 1;
			if (SDreaddata (sensor->sr_id[iband], start, NULL, length, &tmp_sr) ==
					FAILURE)
				{
					fprintf (stderr, "\nOpenLndsrFile():fail to read the SDS band:%d.",
									 iband);
					exit (1);
				}
			else
...
int
openSensorFile (DIS_LANDSAT * sensor)
{
	printf ("Parsing %s...\n", sensor->fileName);
	...
	for (i = 0; i < N_SRF_BAND; i++)
		{
			index = sensor->scene.srf_band_index[i];
			printf ("  %s : %s\n", sensor->scene.band[index].name,
							sensor->scene.band[index].file_name);
	...
	if (0 !=
			get_scene_proj (&(sensor->scene), &sensor->insys, &sensor->inzone,
											&sensor->indatum, sensor->inparm, &sensor->ulx,
	...
	printf
		("NROW=%d, NCOL=%d, FILLV=%d, SCALE=%f, PROJ_SYS=%d, UTM_ZONE=%d, DATUM=%d, ULX=%f, ULY=%f, RES=%f\n",
		 sensor->nrows, sensor->ncols, sensor->SR_fillValue, sensor->lndSR_ScaFct,
		 sensor->insys, sensor->inzone, sensor->indatum, sensor->ulx, sensor->uly,
		 sensor->res);
...
int
openSensorFile_old (DIS_LANDSAT * sensor)
{
	int32 sds_index = 0, sds_id = 0;
	int status, i;
	char sdsName[9][20] =
		{ "band1", "band2", "band3", "band4", "band5", "band7", "band6",
	...
	ret =
		GDprojinfo (gid, &(sensor->insys), &(sensor->inzone), &(sensor->indatum),
								GD_projparm);
	if (ret == GRID_ERRCODE)
		{
			fprintf (stderr,
	...
	ret =
		GDgridinfo (gid, &sensor->ncols, &sensor->nrows, GD_upleft, GD_lowright);
	if (ret == GRID_ERRCODE)
		{
	...
	if (SDreadattr (sensor->sr_id[0], att_id, &(sensor->SR_fillValue)) ==
			FAILURE)
		{
			fprintf (stderr,
	...
	if (SDreadattr (sensor->sr_id[0], att_id, &(sensor->lndSR_ScaFct)) ==
			FAILURE)
		{
			fprintf (stderr,
...
/*Get the scale factor for Landsat band6-Surface Temperature-celcius*/
	...
	if (SDreadattr (sensor->sr_id[6], att_id, &(sensor->lndTemp_ScaFct)) ==
			FAILURE)
		{
			fprintf (stderr,
...
int
cleanUpSensor (DIS_LANDSAT * sensor)
{
	int i;
	int index;
	for (i = 0; i < N_SRF_BAND; i++)
...
/**************************************************************************/
int
getAmongClassesDistance (DIS_LANDSAT * LndWorkspace)
{
	...
	alloc_2dim_contig ((void ***) (&mean_ClsSpec), LndWorkspace->init_ncls,
										 LndWorkspace->nbands, sizeof (float));
		...
		for (j = i + 1; j < LndWorkspace->actu_ncls; j++)
			{
				...
				for (iband = 0; iband < LndWorkspace->nbands; iband++)
					temp_dist +=
						(mean_ClsSpec[i][iband] -
						 mean_ClsSpec[j][iband]) * (mean_ClsSpec[i][iband] -
																				mean_ClsSpec[j][iband]);
				if (temp_dist >= 0)
...
/**************************************************************************/
int
GetEquation4VZACorrection (DIS_LANDSAT * LndWorkspace)
{
	int irow, icol;
	int16 *oneRow;								/*[icol] */
	...
	alloc_1dim_contig ((void **) (&oneRow), LndWorkspace->ncols,
										 sizeof (int16));
	alloc_2dim_contig ((void ***) (&oneBand), LndWorkspace->nrows,
										 LndWorkspace->ncols, sizeof (int16));
	...
	for (irow = 0; irow < LndWorkspace->nrows; irow++)
		{
			...
			if (0 !=
					read_a_band_a_row (&(LndWorkspace->scene.band[index]), irow,
														 (void *) oneRow))
				{
					printf ("Read failed.\n");
					return FAILURE;
	...
	if ((UpperRight[1] == UpperLeft[1]) || (LowerRight[1] == LowerLeft[1])
	...
	LndWorkspace->interception2 =
		UpperLeft[1] - LndWorkspace->slop2 * UpperLeft[0];

	dist_NE =
		sqrt (pow ((UpperLeft[0] - UpperRight[0]), 2) +
					pow ((UpperLeft[1] - UpperRight[1]), 2));
	dist_SE =
		sqrt (pow ((LowerLeft[0] - LowerRight[0]), 2) +
					pow ((LowerLeft[1] - LowerRight[1]), 2));
...
/**************************************************************************/
int
GetEquation4VZACorrection_old (DIS_LANDSAT * LndWorkspace)
{
	int irow, icol;
	/*int iband; */
	int16 *oneRow;								/*[icol] */
	...
	for (irow = 0; irow < LndWorkspace->nrows; irow++)
		{
			...
			for (icol = 0; icol < LndWorkspace->ncols; icol++)
				{
					if ((oneRow[icol] != LndWorkspace->SR_fillValue) && (find_UL == 0)
							&& (find_UR == 0))
						{
							UpperLeft[0] = icol;
							UpperLeft[1] = irow;
					...
					if ((oneRow[icol] != LndWorkspace->SR_fillValue) && (find_UL == 1)
							&& (find_UR == 0))
	...
	for (irow = LndWorkspace->nrows - 1; irow >= 0; irow--)
		{
			...
			for (icol = LndWorkspace->ncols - 1; icol >= 0; icol--)
				{
					if ((oneRow[icol] != LndWorkspace->SR_fillValue) && (find_LR == 0)
							&& (find_LL == 0))
						{
							LowerRight[0] = icol;
							LowerRight[1] = irow;
					...
					if ((oneRow[icol] != LndWorkspace->SR_fillValue) && (find_LR == 1)
							&& (find_LL == 0))
						if (icol < MostLeftInCurrentRow)
							MostLeftInCurrentRow = icol;
				}

			if ((MostLeftInCurrentRow >= MostLeftInLastRow) && (find_LR == 1)
					&& (find_LL == 0))
				{
...
/**************************************************************************/
double
lndSRPixVZA (DIS_LANDSAT * LndWorspace, int row, int col)
{
	double half_dist;
	double pointDist;
	double k, b;
...
/*********************************************************/
double
CalPixVZA (DIS_LANDSAT * LndWorkspace, int irow, int icol)
{
	double VZA, x_ct;
	double PixDist;
	double k, b;
...
/**********************************************************************************/
double
GetVZARefCorrectionFactor (int16 * tmp_brdf, double SZA, double iVZA)
{
	double ref_nadir;
	double ref_vza;
	...
	ref_nadir =
		brdf_scale * (k_iso * tmp_brdf[0] + k_vol * tmp_brdf[1] +
									k_geo * tmp_brdf[2]);
...
/**************************************************/
int
WriteOutHeadFile (DIS_LANDSAT * sensor)
{
...
//  tmp[myStrLen+4]='\0';
	...
	fprintf (fp_hdr,
					 "\nsamples = %d\nlines   = %d\nbands  = 2\nheader offset = 0",
					 sensor->ncols, sensor->nrows);
	fprintf (fp_hdr,
					 "\nfile type = ENVI Standard\ndata type = 2\ninterleave = bip\nsensor type = Landsat");
	fprintf (fp_hdr,
					 "\nbyte order = 0\nwavelength units = Shortwave band 300-2500nm");
	fprintf (fp_hdr,
					 "\nband names ={\n\t(%dX%d):Shortwave band black sky albedo: Band 1,",
					 sensor->ncols, sensor->nrows);
	fprintf (fp_hdr, "\n\t(%dX%d):Shortwave band white sky albedo: Band 2}",
					 sensor->ncols, sensor->nrows);
	fprintf (fp_hdr, "\nmap info={UTM, 1.000, 1.000, %f, %f, %f, %f, ",
					 sensor->ulx, sensor->uly, sensor->res, sensor->res);
...
// snow: return 2
int
CloudMask (DIS_LANDSAT * LndWorkspace, int icol)
{
	int temp;
	int bitp[32];
...
int
SnowDetermination (DIS_LANDSAT * LndWorkspace, int icol)
{
	double NDSI;
	double NDVI, diff;
...
/*************************************************/
int
getClosestCls (DIS_LANDSAT * LndWorkspace, int icls, int band)
{
	int mycls;
	float min_dist;
	int i;
	...
	for (i = 0; i < LndWorkspace->actu_ncls; i++)
		{
			brdfNotAva = 0;
			if ((LndWorkspace->BRDF_paras[i][band][0] == 0)
					&& (LndWorkspace->BRDF_paras[i][band][1] == 0)
					&& (LndWorkspace->BRDF_paras[i][band][2] == 0))
				brdfNotAva = 1;

			if ((icls != i) && (LndWorkspace->AmongClsDistance[icls][i] < min_dist)
					&& (brdfNotAva == 0))
				{
...
double
CalPixAzimuth (DIS_LANDSAT * LndWorkspace, int irow, int icol)
...
/**************************************************************************/
int
CalcScanCenters (DIS_LANDSAT * LndWorkspace)
{
	int i;
	double delta_col, delta_row;

	delta_col =
		(LndWorkspace->SE_center[0] - LndWorkspace->NE_center[0]) * 1.0 / NSCAN;
	delta_row =
		(LndWorkspace->SE_center[1] - LndWorkspace->NE_center[1]) * 1.0 / NSCAN;
	if ((LndWorkspace->SE_center[0] == LndWorkspace->NE_center[0])
			|| (LndWorkspace->SE_center[1] == LndWorkspace->NE_center[1]))
		return FAILURE;

	LndWorkspace->scan_centers[0][0] =
		LndWorkspace->NE_center[0] + 0.5 * delta_col;
	LndWorkspace->scan_centers[1][0] =
		LndWorkspace->NE_center[1] + 0.5 * delta_row;

	for (i = 1; i < NSCAN; i++)
		{
			LndWorkspace->scan_centers[0][i] =
				LndWorkspace->scan_centers[0][i - 1] + delta_col;
			LndWorkspace->scan_centers[1][i] =
				LndWorkspace->scan_centers[1][i - 1] + delta_row;
		}

	LndWorkspace->oneScanDist_alongNadir =
		sqrt (delta_col * delta_col + delta_row * delta_row);
...
int
CalcScanCenters_old (DIS_LANDSAT * LndWorkspace)
{
	int i;
	...
	//first line center
	scan_center[0] =
		(LndWorkspace->img_upright[0] + LndWorkspace->img_upleft[0]) / 2;
	scan_center[1] =
		(LndWorkspace->img_upright[1] + LndWorkspace->img_upleft[1]) / 2;

	printf ("r_step=%f\n", r_step);
	printf ("img_upleft=%d, %d, upright=%d, %d\n", LndWorkspace->img_upleft[0],
					LndWorkspace->img_upleft[1], LndWorkspace->img_upright[0],
					LndWorkspace->img_upright[1]);
	printf ("first line center=%f, %f\n", scan_center[0], scan_center[1]);
	printf ("slop1=%f, interc=%f\n", LndWorkspace->slop,
					LndWorkspace->interception);
	...
	for (i = 0; i < NSCAN; i++)
		{
			...
			scan_center[0] =
				(p_y - LndWorkspace->interception) / LndWorkspace->slop;
			scan_center[1] = p_y;
