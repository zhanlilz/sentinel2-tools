...
int
loadModisPixel (GRID_MODIS * modis, int irow, int icol)
{
	int iband;
	int32 start[3];
	int32 length[3];
	...
	for (iband = 0; iband < MODIS_NBANDS; iband++)
		{
			if (SDreaddata (modis->brdf_id[iband], start, NULL, length, val_temp) ==
					FAILURE)
				{
	...
	if (modis->qa_id != FAILURE && modis->qa_id != SUCCESS)
		{
			if (SDreaddata (modis->qa_id, start, NULL, length, &modis->Pix_qa) ==
					FAILURE)
				{
	...
	/* sqs */
	else if (modis->qa_id == SUCCESS)
		{
			...
			for (iband = 0; iband < MODIS_NBANDS; iband++)
				{
					if (SDreaddata (modis->qa_ids[iband], start, NULL, length, &tt) ==
							FAILURE)
						{
							fprintf (stderr,
											 "\nloadModisRow: reading BRDF band QA error\n");
...
int
loadModisRow (GRID_MODIS * modis, int irow)
{
	int iband;
	int32 start[2];
	int32 length[2];
	...
	for (iband = 0; iband < MODIS_NBANDS; iband++)
		if (SDreaddata
				(modis->brdf_id[iband], start, NULL, length,
				 modis->sr[iband]) == FAILURE)
			{
				fprintf (stderr, "\nloadModisRow: reading reflectance error\n");
				return FAILURE;
			}
	...
	if (modis->snow_id != FAILURE)
		if (SDreaddata (modis->snow_id, start, NULL, length, modis->snow) ==
				FAILURE)
			{
...
int
getModisMetaInfo (GRID_MODIS * modis)
{
	char name[100];
	int i, index, ret;
	...
	int32 att_id;
	char sdsName[7][100] =
		{ "BRDF_Albedo_Parameters_Band1", "BRDF_Albedo_Parameters_Band2",
	...
	/* get grid info */
	ret =
		GDgridinfo (gid, &(modis->ncols), &(modis->nrows), modis->GD_upleft,
								modis->GD_lowright);
	if (ret == FAILURE)
		{
			fprintf (stderr,
	...
	ret =
		GDprojinfo (gid, &modis->GD_projcode, &modis->GD_zonecode,
								&modis->GD_spherecode, modis->GD_projparm);
	if (ret == FAILURE)
		{
			fprintf (stderr,
	...
	if (ret == FAILURE)
		{
			fprintf (stderr, "\nFailed to detach grid for %s: getModisMetaInfo",
							 modis->srName);
			return FAILURE;
		}
	...
	if (ret == FAILURE)
		{
			fprintf (stderr, "\nGD-file close failed for %s: getModisMetaInfo",
							 modis->srName);
			return FAILURE;
		}
	...
	alloc_2dim_contig ((void ***) (&modis->sr), MODIS_NBANDS, modis->ncols,
										 sizeof (int16));
	alloc_1dim_contig ((void **) (&modis->qa), modis->ncols, sizeof (uint32));
	alloc_1dim_contig ((void **) (&modis->snow), modis->ncols, sizeof (int8));

	alloc_2dim_contig ((void ***) (&modis->major_cls), modis->nrows,
										 modis->ncols, sizeof (uint8));
	alloc_2dim_contig ((void ***) (&modis->percent_cls), modis->nrows,
										 modis->ncols, sizeof (uint8));
