...
main (int argc, char *argv[])
{
	...
	char *usage =
		"usage: exe MODIS_dir MCD43A1/MCD43A2 hdf_out nXml xml1 xml2 ... xmln";
...
main1 (int argc, char *argv[])
{
	...
	if (0 !=
			get_scene_proj (&scene, &sys, &zone, &datum, parm, &ulx, &uly, &res))
		{
	...
	printf ("year=%d, doy=%d, nrow=%d, ncol=%d, band 0 index=%d\n", year, doy,
					scene.band[0].nrow, scene.band[0].ncol, index);
	printf ("proj sys=%d, zone=%d, datum=%d, ulx=%lf, uly=%lf, res=%lf\n", sys,
					zone, datum, ulx, uly, res);
	...
	assert (0 ==
					SetupSpace (PROJ_NUM, UTM_ZONE, PROJ_PARAM, SPHERE, UL_X, UL_Y,
											PIX_SIZE));
...
int
find_modis_region (char *xml, Region * rModis, int *year, int *doy)
{
	usgs_t scene;
	long sys, zone, datum;
	double parm[15], ulx, uly, res;
...
void
global2tile (Point * gpoint, TilePoint * tpoint)
{
...
void
geo2grid (double lat, double lon, Point * point)
{
	double l, s;
...
int
get_hdf_data (Point * point, int year, int doy, char *result)
{
...
int
_get_hdf_data (int h, int v, int year, int doy, char *result)
{
	char cmd[2048];
	char tile[10];
	sprintf (tile, "h%02dv%02d", h, v);
	sprintf (cmd,
					 "find %s -name \"%s.A%d%03d.%s.*hdf\" > /tmp/tmpfile_sqs_1231232323",
					 Data_dir, Data_head, year, doy, tile);
	system (cmd);
...
int
sametile (TilePoint * tp1, TilePoint * tp2)
{
...
int
new_job (char *hdf_source, int32 start_source[], int32 edge_source[],
				 char *hdf_target, int32 start_target[], int32 edge_target[])
{
	Job job;
...
int
do_copy_sub_hdf (char *hdf_source, int32 start_source[], int32 edge_source[],
								 char *hdf_target, int32 start_target[], int32 edge_target[])
{
	char cmd[1024];
	sprintf (cmd, "%s/copy %s %s %d %d %d %d %d %d %d %d", path, hdf_source,
					 hdf_target, start_source[0], start_source[1], edge_source[0],
					 edge_source[1], start_target[0], start_target[1], edge_target[0],
					 edge_target[1]);
	assert (0 == system (cmd));
	return 0;
}
...
int
mosaic_cut_new (char *hdf_out, double lat, double lon, int year, int doy)
{
	Point center;
			...
			for (v = tp1.v; v <= tp4.v; v++)
				{
					...
					if (v == tp1.v)
						...
						{
							start_out[0] =
								(TILE_NROW - tp1.row) + (v - tp1.v - 1) * TILE_NROW;
						}
					...
					if (h == tp1.h)
						...
						{
							start_out[1] =
								(TILE_NCOL - tp1.col) + (h - tp1.h - 1) * TILE_NCOL;
						}
					...
					if (0 == _get_hdf_data (h, v, year, doy, hdf_in))
						{
							assert (0 ==
											new_job (hdf_in, start, edge, hdf_out, start_out,
															 edge_out));
						}
					else if (!fill_flag)
						{
						...
						{
							sprintf (msg,
											 "Warning: h%02dv%02d is missing. flag -f detected.\n",
											 h, v);
							WARN (msg);
						}
				}
...
int
mosaic_cut_new2 (char *hdf_out, Region * rModis, int year, int doy)
{
	int32 start[2];
	int32 edge[2];
	int32 start_out[2];
...
int
mosaic_cut (char *hdf_out, double lat, double lon, int year, int doy)
{
	Point center;
	geo2grid (lat, lon, &center);
	...
	printf
		("CENTER: LAT %f LON %f GLOBAL ROW  = %d GLOBAL COL = %d, TILE=h%02dv%02d, %d, %d\n",
...
void
make_region (Point * center, Region * region)
{
...
submit_jobs ()
{
	...
	for (i = 0; i < N_JOB; i++)
		{
			printf ("Copy %s:\n  col %4d - %4d\n  row %4d - %4d\n",
							JOBS[i].fname_s, JOBS[i].start_s[1],
							JOBS[i].start_s[1] + JOBS[i].edge_s[1] - 1, JOBS[i].start_s[0],
							JOBS[i].start_s[0] + JOBS[i].edge_s[0] - 1);
			printf ("To   %s:\n  col %4d - %4d\n  row %4d - %4d\n", JOBS[i].fname_t,
							JOBS[i].start_t[1], JOBS[i].start_t[1] + JOBS[i].edge_t[1] - 1,
							JOBS[i].start_t[0], JOBS[i].start_t[0] + JOBS[i].edge_t[0] - 1);
			assert (0 ==
							do_copy_sub_hdf (JOBS[i].fname_s, JOBS[i].start_s,
															 JOBS[i].edge_s, JOBS[i].fname_t,
															 JOBS[i].start_t, JOBS[i].edge_t));
		}
