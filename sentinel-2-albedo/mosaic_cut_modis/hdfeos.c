...
int copy_field(int32 dstGd, int32 tgtGd, char *fieldName, int nrow, int ncol)
{
	int32 rank;
	int32 dims[10];
	int32 numType;
	...
	// copy info
	if(SUCCEED != GDfieldinfo(dstGd, fieldName, &rank, dims, &numType, dimList)){
	...
	if(SUCCEED != GDgetfillvalue(dstGd, fieldName, (VOIDP)&fillv)){
		...
		printf("Warning: Get fill value failed for %s. Using default value: %d\n", fieldName, fillv);
	}
	...
	if(SUCCEED != GDtileinfo(dstGd, fieldName, &tileCode, &tileRank, tileDims)){
		printf("Error! Inqury comp info failed.\n");
		return -1;
	}
	...
	if(SUCCEED != GDdeffield(tgtGd, fieldName, dimList, numType, HDFE_NOMERGE)){
...
int copy_field_attr(int32 sdId_dst, int32 sdId_tgt, char *fieldName)
{
	// copy attr
	int i;
	int32 rank;
	...
	if(SUCCEED != SDgetinfo(sdsId_dst, sdsName, &rank, dims, &numType, &numAttrs)){
	...
	for(i=0; i<numAttrs; i++){
		...
		if(SUCCEED != SDsetattr(sdsId_tgt, attrName, numType, count, (VOIDP)attrBuf)){
			printf("Error! Read attr failed.\n");
			return -1;
		}
	}
...
int create_output(char *hdf_sample, char *hdf_out, Region *region)
{
	...
	float64 upleft[2] = {UL_X + PIX_SIZE * region->upper_left.col, UL_Y - PIX_SIZE * region->upper_left.row};
	float64 lowright[2] = {upleft[0] + ncol*PIX_SIZE, upleft[1] - PIX_SIZE*nrow};
